...
{
  _INVOKE_VA(args, _scan_kv_pairs_fn(scanner, input, _compare_key_value, args));
}
...
{
  KVScanner *scanner = kv_scanner_new();
  _INVOKE_VA(args, _scan_kv_pairs_fn(scanner, input, fn, args));
  kv_scanner_free(scanner);
}
