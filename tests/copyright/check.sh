#!/bin/bash
DEBUG="3" # 0..4, 0=least amount of output
...
main_logged() {
 ...
 -type f -print |
 sed "s~^\./~~" |
 grep --extended-regexp "\.(c|h|am|md|ym)$" |
 prune_ignored_paths |
 sort |
 while read FILE; do
 ...
 if [ -z "$HOLDER" -a -z "$LICENSE" ]; then
  {
   ...
   then
    printf "error: can't parse $FILE"
   else
    printf "error: missing license $FILE"
   fi
   if [ -n "$PREVIEW" ]; then
    printf " "
    cat "$FILE"
  ...
  escaped_preview >&2
  if [ -z "$PATHMATCH" ]; then
   echo "error: unmatched path $FILE" >&2
  else
   if [ "0$DEBUG" -ge 1 ]; then
    echo "info: $FILE path expects $PATHMATCH" >&2
   fi
  fi
 else
  if [ "0$DEBUG" -ge 4 ]; then
   {
    printf "$FILE\n" >&2
...
got_both_holder_and_license() {
   ...
   if [ -n "$PATHMATCH" -a -n "$LICTEXTMATCH" ]; then
    ...
    if [ -z "$PATHMATCH" ]; then
     ...
     if [ -n "$LICTEXTMATCH" -a "0$DEBUG" -ge 1 ]; then
    ...
    if [ -z "$LICTEXTMATCH" ]; then
     {
      printf "error: unknown license $FILE"
      if [ -n "$PREVIEW" ]; then
       echo " $LICENSE"
      fi
     } |
     ...
     if [ -n "$PATHMATCH" -a "0$DEBUG" -ge 1 ]; then
      echo "info: $FILE path expects $PATHMATCH" >&2
     fi
    fi
   fi
  ...
  case "$KIND" in
   ...
   P)
    if [ -n "$LICENSE" ]; then
     ...
     echo "error: ignored unknowned licensed path '${LINE}'" >&2
     local ERR=1
    fi
    ;;
   *)
