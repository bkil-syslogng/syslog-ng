 ...
 */
void
append_unsafe_utf8_as_escaped_binary(GString *escaped_string, const GString *str, const gchar *unsafe_chars)
{
...
void
append_unsafe_utf8_as_escaped_binary_c (GString *escaped_string,
                                        const gchar *str,
                                        const gchar *unsafe_chars)
{
  GString *gstr = g_string_new (str);
  append_unsafe_utf8_as_escaped_binary (escaped_string, gstr, unsafe_chars);
  g_string_free (gstr, FALSE);
}

gchar *
convert_unsafe_utf8_to_escaped_binary_c(const gchar *str, const gchar *unsafe_chars)
{
  GString *original = g_string_new(str);
  GString *escaped_string = g_string_sized_new(original->len);

  append_unsafe_utf8_as_escaped_binary(escaped_string, original, unsafe_chars);
  g_string_free(original, FALSE);
 ...
 **/
void
append_unsafe_utf8_as_escaped_text(GString *escaped_string, const GString *str, const gchar *unsafe_chars)
{
  const gchar *char_ptr = str->str;
  const gchar *const end = char_ptr + str->len;
...
void
append_unsafe_utf8_as_escaped_text_c (GString *escaped_string, const gchar *str,
                                      const gchar *unsafe_chars)
{
  GString *gstr = g_string_new (str);
  append_unsafe_utf8_as_escaped_text (escaped_string, gstr, unsafe_chars);
  g_string_free (gstr, FALSE);
}

gchar *
convert_unsafe_utf8_to_escaped_text_c(const gchar *str, const gchar *unsafe_chars)
{
  GString *original = g_string_new(str);
  GString *escaped_string = g_string_sized_new(original->len);

  append_unsafe_utf8_as_escaped_text(escaped_string, original, unsafe_chars);
  g_string_free(original, FALSE);
  return g_string_free(escaped_string, FALSE);
}
